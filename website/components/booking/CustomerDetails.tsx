...
const CustomerDetails: React.FC<CustomerDetailsProps> = ({ onNext, onBack }) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Select onValueChange={(value) => register('passengers').onChange({ target: { value } })}>
            <SelectTrigger>
          ...
          {errors.passengers && <p className="text-red-500 text-sm mt-1">{errors.passengers.message as string}</p>}
        </div>

        <div>
          <Label htmlFor="luggage">Total Luggage</Label>
          <Select onValueChange={(value) => register('luggage').onChange({ target: { value } })}>
            <SelectTrigger>
          ...
          {errors.luggage && <p className="text-red-500 text-sm mt-1">{errors.luggage.message as string}</p>}
        </div>
      </div>
